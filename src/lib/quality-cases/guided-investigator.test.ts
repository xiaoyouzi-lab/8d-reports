...
import { buildGuidedInvestigatorPrompt, classifyGuidedContractIssues, validateGuidedInvestigatorResponse } from "@/lib/quality-cases/guided-investigator";
...
assert.deepEqual(classifyGuidedContractIssues(["missingConcepts is invalid"]), ["missing_concepts_enum"]);
assert.deepEqual(classifyGuidedContractIssues(["confirmedRootCause is not permitted", "workflowAction is not permitted"]), ["forbidden_field"]);
