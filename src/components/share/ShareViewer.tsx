...
// Shared renderer for the tokenized report viewer. Both /share/[token] and
// /zh/share/[token] render this component; the active locale comes from the URL
// through the root LocaleProvider, so the copy below is the only difference.
export function ShareViewer({ token }: { token: string }) {
  const t = useTranslations("share")
  // D0-D8 step titles already exist (and are translated) under docs.step.*.name,
  // so the viewer reuses them instead of inventing new 8D terminology.
  ...
  const approvals = [
    { key: "preparedBy", label: t("preparedBy"), name: displayData.preparedBy, date: displayData.preparedDate, url: displayData.preparedSignatureUrl },
    { key: "reviewedBy", label: t("reviewedBy"), name: displayData.reviewedBy, date: displayData.reviewedDate, url: displayData.reviewedSignatureUrl },
    { key: "approvedBy", label: t("approvedBy"), name: displayData.approverName, date: displayData.approverDate, url: displayData.approvedSignatureUrl },
  ] as const
                ...
                return (
                  <div key={item.key} className="rounded-lg border bg-white p-3">
                    ...
                    <div className="mt-3 flex h-20 items-center justify-center rounded border border-dashed bg-slate-50">
                      {item.url ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={signatureUrl(item.url)} alt={t("signatureAlt", { name: label })} className="max-h-16 max-w-full object-contain" />
                      ) : (
                        <span className="text-xs text-muted-foreground">{t("signatureLine")}</span>
                      )}
                    </div>
                  </div>
                )
