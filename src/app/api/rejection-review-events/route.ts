...
const PUBLIC_EVENTS = new Set(["review_landing_view", "review_upload_started", "review_free_result_viewed"]);
...
export async function POST(req: NextRequest) {
  ...
  await recordRejectionReviewEvent({
    ...
    actorKind: user ? classifyReviewActor({ email: user.email }) : "anonymous",
    trafficSource,
    userId: user?.id || null,
    taskId,
    metadata: { locale: body.locale === "zh-CN" ? "zh-CN" : "en" },
    dedupeKey: `${eventName}:${authoritativeSessionHash}:${eventId}`,
  });
