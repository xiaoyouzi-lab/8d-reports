...
export async function POST(req: NextRequest) {
  ...
  if (access.locked) {
    return NextResponse.json({ error: "This report is locked. Unlock it for revision before generating an AI draft." }, { status: 403 });
  }
  if (!access.canEdit) {
    return NextResponse.json({ error: "You do not have permission to generate an AI draft for this report." }, { status: 403 });
  }
  const { report } = access;
