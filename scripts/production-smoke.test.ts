...
async function main() {
  await expectHtml("/", ["Create, approve, lock, and deliver", "Team Launch"]);
  await expectHtml("/pricing", ["Team", "$99", "Owner / Editor / Viewer roles"]);
  await expectHtml("/team-launch", ["Launch your online 8D workflow in 7 days", "From $999"]);
  await expectHtml("/custom-8d-template-setup", ["reusable online workflow", "From $499"]);
  await expectHtml("/demo-reports", ["Team workflow demos", "Download delivery ZIP"]);
