...
`)
  ...
  writeFile("ops/publishing/help-asset-refresh.md", `
# Help Asset Refresh

## Purpose

Use this internal publishing note when Help Center screenshots need to be refreshed before public publishing. Public Help and Learn pages should never describe capture tooling, local paths, storage state files, report ids, or asset maintenance steps.

## Current v1 State

- Content Ops v1 uses seeded screenshots from existing product audit material.
- The seeded screenshots make the Help Center readable while avoiding unsafe production data writes or fake authenticated content.
- Some AI and workflow screenshots may show the editor context rather than a fully opened result panel until a safe authenticated refresh is available.

## Refresh Command

Prepare a safe local authenticated browser storage state and a non-production report id, then run:
...
- Do not commit credentials, cookies, tokens, database URLs, or private report content.
...
Public Help and Learn pages may say that a screenshot shows where a feature appears in the workflow. They must not mention Playwright, capture scripts, storage state, report ids, local paths, seeded screenshots, repository paths, or ops maintenance instructions.
`)
