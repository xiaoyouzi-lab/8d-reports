...
- Added **all 50 programmatic SEO pages in Simplified Chinese** across the 6
  route groups generated by `src/content/seo-pages.ts`: 12
  `/zh/8d-report-example/*`, 10 `/zh/8d-report-template/*`, 10
  `/zh/5-why-example/*`, 6 `/zh/fishbone-diagram-example/*`, 6
  `/zh/corrective-action-example/*`, and 6 `/zh/preventive-action-example/*`.
  The English module is untouched and stays the source of truth for slugs,
  types, and structure.
- Added `src/content/seo-pages-zh.ts` as a mechanical mirror of the English
  generator. It translates the shared Scenario field values once (12 base
  example scenarios plus the late-delivery, process-failure, and quality-system
  scenarios), translates every static template string used by
  `buildRelated` / `buildEightDSteps` / `buildFiveWhy` / `buildFishbone` /
  `buildActionPlan` / `buildPreventionPlan` / `buildZhPage`, and rebuilds the
  same derived arrays (template, 5Why, fishbone, corrective, preventive) with
  the same slugs and `problemType` overrides. It exports `buildZhPage`,
  `seoPagesZh`, `getSeoPageZh`, `getSeoPagesByTypeZh`, and
  `getRelatedSeoPagesZh`.
- Added `src/components/seo/SeoLandingPageZh.tsx`, a Chinese rendering of the
  `SeoLandingPage` sections (D0-D8 sample table, owner/evidence columns,
  attachment and closure checklists, 5Why, fishbone 6M, action plans, FAQ,
  related resources). It emits zh-CN JSON-LD URLs and only sells live product
  features: D0-D8 editing, evidence/attachments, PDF/Word/Excel + ZIP export,
  share links, Team roles/approval/lock/activity, Knowledge Base reuse, and the
  automated AI Quality Check.
- Added `src/lib/seo-route-zh.tsx` and the 6 thin
  `src/app/zh/<group>/[slug]/page.tsx` route files. Each sets
  `dynamicParams = false`, `generateStaticParams` from the zh pages of its
  type, and `notFound()` for untranslated slugs. Metadata uses the Chinese
  title/description, a `https://www.8d-reports.com/zh/<group>/<slug>` canonical,
  and `alternates.languages` for `en` + `zh-CN`.
- Extended `src/lib/i18n-routes.ts` with `ZH_SEO_EXAMPLE_SLUGS`,
  `ZH_SEO_TEMPLATE_SLUGS`, `ZH_SEO_FIVE_WHY_SLUGS`, `ZH_SEO_FISHBONE_SLUGS`,
  `ZH_SEO_CORRECTIVE_SLUGS`, and `ZH_SEO_PREVENTIVE_SLUGS` plus the 6 matching
  `ZH_DYNAMIC_COLLECTIONS` entries, so
  `/<group>/<slug> <-> /zh/<group>/<slug>` resolves for every generated slug.
  The existing static map and the resources/learn/help/docs collections are
  unchanged.
- Added zh-CN hreflang to the shared English programmatic metadata in
  `src/lib/seo-route.tsx`, and extended `src/app/sitemap.ts` so all 50 English
  programmatic URLs carry two-way `xhtml:link` alternates and all 50
  `/zh/...` programmatic URLs are emitted with their own alternates.
- Updated `scripts/check-seo-urls.ts` to know the zh programmatic content paths.
- Added `scripts/i18n-seo.test.ts` (`npm run test:i18n-seo`) asserting: zh page
  set count/slugs/types match English (50 pages / 6 groups / 12-10-10-6-6-6);
  required zh fields are non-empty; section, D-step, 5Why, fishbone, action
  plan, prevention plan, example, FAQ, and related slugs have full structural
  parity; the 6 zh routes exist with the static-param and 404 guards; both route
  directions map and localize for every slug; canonical and hreflang are correct
  in both directions; sitemap alternates exist both ways; and the forbidden
  Quality Case / complaint-workbench / supplier-collaboration / manual or expert
  review / custom-service positioning and removed routes never appear.
- No auth, payment, database schema, export logic, environment variable, or
  production configuration change.
...
- `npm run test:governance`, `test:title-hygiene`,
  `test:tool-only-positioning`, `test:i18n-core`, `test:i18n-resources`,
  `test:i18n-learn-help`, `test:i18n-docs`, and `test:i18n-seo` passed.
- `npm run check:seo` passed with 229 sitemap URLs and 16 redirects.
- `npm run build` passed.

### Residual risk (Batch 3)

- The Chinese copy is a first professional translation pass and should get a
  native quality-engineering review before a dedicated zh marketing push.
- The zh programmatic pages reuse the shared `SeoPrimaryCta` / `SeoTemplateCta`
  client components, which point at the existing `/signup` and `/reports/new`
  product routes rather than localized auth routes.
